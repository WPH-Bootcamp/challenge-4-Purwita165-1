...
class Student {
  constructor({ id, name, class: studentClass, grades = {} }) {
    if (!id || !name || !studentClass) {
      throw new Error('ID, Nama, dan Kelas wajib diisi.');
    }

    this.id = String(id).trim();
    this.name = String(name).trim();
    this.class = String(studentClass).trim();
    this.grades = { ...grades }; // object { mataPelajaran: nilai }
  }
  ...
  addGrade(subject, score) {
    ...
    if (!subj) throw new Error('Nama mata pelajaran wajib diisi.');
    if (!Number.isFinite(num) || num < 0 || num > 100) {
      throw new Error('Nilai harus berupa angka antara 0 dan 100.');
    }
    this.grades[subj] = num;
  }
   ...
   * @returns {string} Informasi terformat
   */
  displayInfo() {
    const subjects =
      Object.keys(this.grades).length === 0
        ? '  (belum ada nilai)'
        : Object.entries(this.grades)
    ...
    return [
      ...
      `Mata Pelajaran:`,
      subjects,
      `Rata-rata: ${this.getAverage()}`,
      `Status: ${this.getGradeStatus()}`,
    ].join('\n');
  }

  // Getter tambahan untuk dipakai StudentManager
  getId() {
    return this.id;
  }
  ...
  setName(newName) {
    if (!newName || !newName.trim()) throw new Error('Nama tidak boleh kosong.');
    this.name = newName.trim();
  }

  setClass(newClass) {
    if (!newClass || !newClass.trim()) throw new Error('Kelas tidak boleh kosong.');
